...
importance.to_csv(OUTPUTS / "xgb_feature_importance.csv", index=False)
predictions.to_csv(OUTPUTS / "xgb_test_predictions.csv", index=False)
joblib.dump(model, OUTPUTS / "xgb_brent_model.pkl")
joblib.dump(scaler, OUTPUTS / "xgb_feature_scaler.pkl")
...
ax.legend()
fig.tight_layout()
fig.savefig(OUTPUTS / "xgb_test_predictions.png", dpi=160)
plt.close(fig)

with (OUTPUTS / "xgb_results.json").open("w", encoding="utf-8") as f:
    json.dump(result, f, indent=2)

print(json.dumps(result, indent=2))
print("\nTop 15 features:")
print(importance.head(15).to_string(index=False))
print(f"\nSaved: {OUTPUTS / 'xgb_results.json'}")
print(f"Saved: {OUTPUTS / 'xgb_test_predictions.csv'}")
print(f"Saved: {OUTPUTS / 'xgb_feature_importance.csv'}")
print(f"Saved: {OUTPUTS / 'xgb_test_predictions.png'}")
