...
model = AttentionBiLSTM(input_size=len(features), hidden_size=32, dropout=0.15)
...
lag_attention = pd.DataFrame(
    {
        ...
        "mean_attention_weight": attn_test.numpy().mean(axis=0),
    }
)
...
predictions.to_csv(OUTPUTS / "attention_bilstm_test_predictions.csv", index=False)
lag_attention.to_csv(OUTPUTS / "attention_bilstm_lag_attention.csv", index=False)
history_df.to_csv(OUTPUTS / "attention_bilstm_history.csv", index=False)
torch.save(model.state_dict(), OUTPUTS / "attention_bilstm_model.pt")
joblib.dump(scaler, OUTPUTS / "attention_bilstm_feature_scaler.pkl")
...
ax.legend()
fig.tight_layout()
fig.savefig(OUTPUTS / "attention_bilstm_test_predictions.png", dpi=160)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(OUTPUTS / "attention_bilstm_lag_attention.png", dpi=160)
plt.close(fig)

with (OUTPUTS / "attention_bilstm_results.json").open("w", encoding="utf-8") as f:
...
print(f"\nSaved: {OUTPUTS / 'attention_bilstm_results.json'}")
print(f"Saved: {OUTPUTS / 'attention_bilstm_test_predictions.csv'}")
print(f"Saved: {OUTPUTS / 'attention_bilstm_lag_attention.csv'}")
print(f"Saved: {OUTPUTS / 'attention_bilstm_test_predictions.png'}")
print(f"Saved: {OUTPUTS / 'attention_bilstm_lag_attention.png'}")
